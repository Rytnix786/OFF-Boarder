...
export async function getRiskRadarDashboard(filters: {
  ...
  const [offboardings, total, alerts, ghostExits, accessRevocationsCount] = await withPrismaRetry(() => Promise.all([
  ...
  const [criticalCount, highCount, unresolvedAssetsCount] = await withPrismaRetry(() => Promise.all([
...
export async function getRiskRadarCaseDetail(offboardingId: string) {
  ...
  const offboarding = await withPrismaRetry(() => prisma.offboarding.findFirst({
    where: { id: offboardingId, organizationId: orgId },
    include: {
      employee: {
  ...
  const [riskScore, accessRevocations, securityEvents, auditLogs] = await withPrismaRetry(() => Promise.all([
