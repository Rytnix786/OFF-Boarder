...
export async function AdminDashboard({ session }: AdminDashboardProps) {
  ...
  const sevenDaysAgo = new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000);
    ...
    const onboardingStatus = await withPrismaRetry(() => getOnboardingStatus(orgId), { retries: 2, baseDelayMs: 250 });

    const recentCompletions = recentCompletionsInDb.slice(0, 5);
  ...
  const getDaysOverdue = (dueDate: Date | string) => {
    ...
    const orgName = session.currentMembership?.organization.name || "Organization";
      ...
      return (
        ...
        {/* ═══════════════════════════════════════════════════════════════════════════════════════
            1) GLOBAL SYSTEM STATUS (TOP PRIORITY – FULL WIDTH)
            Purpose: High-level system health and risk indicators
        ═══════════════════════════════════════════════════════════════════════════════════════ */}
        <Box sx={{ mb: 6 }}>
          {/* Posture Card */}
          <Card
        ...
        {/* ═══════════════════════════════════════════════════════════════════════════════════════
            2) ACTION REQUIRED (PRIMARY ADMIN FOCUS – IMMEDIATE VISIBILITY)
            Purpose: Critical controls and actionable alerts
        ═══════════════════════════════════════════════════════════════════════════════════════ */}
        <Box sx={{ mb: 6 }}>
          ...
          {/* Actionable Admin Controls Grid */}
          <Grid container spacing={2} sx={{ mb: 3 }}>
            {[
              { label: "Initiate Offboarding", href: "/app/offboardings", icon: "person_remove", warning: !onboardingStatus.isComplete ? "COMPLETE ONBOARDING FIRST" : null },
              { label: "Employee Directory", href: "/app/employees", icon: "badge" },
              { label: "Audit Trail", href: "/app/audit-logs", icon: "receipt_long" },
              { label: "Compliance Reports", href: "/app/reports", icon: "assessment" },
            ].map((control) => (
              <Grid key={control.label} size={{ xs: 12, sm: 6, md: 3 }}>
                <Link href={control.href} style={{ textDecoration: "none", color: "inherit" }}>
                  <Card 
                    variant="outlined" 
                    sx={{ 
                      borderRadius: 2, 
                    ...
                    <CardContent sx={{ p: 2, display: "flex", alignItems: "center", gap: 2 }}>
                      <Box
                        sx={{
                          ...
                          height: 36,
                          borderRadius: 1,
                          bgcolor: alpha(control.warning ? "#d97706" : "#3b82f6", 0.08),
                          display: "flex",
                          alignItems: "center",
                          justifyContent: "center",
                          flexShrink: 0,
                      ...
                      >
                        <span className="material-symbols-outlined" style={{ fontSize: 20, color: control.warning ? "#d97706" : "#3b82f6" }}>
                          {control.icon}
                        </span>
                      </Box>
                      <Box sx={{ minWidth: 0 }}>
                        ...
                        {control.warning && (
                          <Typography variant="caption" sx={{ color: "#d97706", fontWeight: 800, fontSize: "0.625rem", display: "block" }}>
                            {control.warning}
                          </Typography>
                        )}
                      </Box>
          ...
          {/* Actionable Alerts (Overdue, High-Risk, etc.) */}
        ...
        {/* ═══════════════════════════════════════════════════════════════════════════════════════
            3) SECURITY & ACTIVITY FEED (INFORMATIONAL, NOT ACTIONABLE)
            Purpose: Passive monitoring of system events and audit signals
        ═══════════════════════════════════════════════════════════════════════════════════════ */}
        <Box sx={{ mb: 6 }}>
          <Typography variant="overline" sx={{ color: "text.secondary", fontWeight: 700, letterSpacing: 1, mb: 2, display: "block" }}>
            ...
            <Box sx={{ maxHeight: 400, overflow: "auto" }}>
              {recentSecurityEvents.length === 0 ? (
                <Box sx={{ p: 4, textAlign: "center" }}>
                  <Typography variant="body2" color="text.secondary">No recent security events</Typography>
                </Box>
              ) : (
        ...
        {/* ═══════════════════════════════════════════════════════════════════════════════════════
            ...
            Purpose: Pipeline health, metrics, and throughput monitoring
        ═══════════════════════════════════════════════════════════════════════════════════════ */}
        <Box sx={{ mb: 6 }}>
          <Typography variant="overline" sx={{ color: "text.secondary", fontWeight: 700, letterSpacing: 1, mb: 2, display: "block" }}>
            Operational Status
        ...
        {/* ═══════════════════════════════════════════════════════════════════════════════════════
            5) COMPLIANCE & ASSURANCE (TRUST & AUDIT READINESS)
            Purpose: Evidence packs, audit logs, and system assurance
        ═══════════════════════════════════════════════════════════════════════════════════════ */}
        <Box>
          <Typography variant="overline" sx={{ color: "text.secondary", fontWeight: 700, letterSpacing: 1, mb: 2, display: "block" }}>
